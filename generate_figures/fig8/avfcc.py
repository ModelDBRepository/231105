...
xval_min_fr = float(sys.argv[3])
xval_min_cv = float(sys.argv[4])
xval_min_ch = float(sys.argv[5])
print 'suffix=', suffix, 'factor=', factor, ' xval_min=', xval_min_fr, xval_min_cv, xval_min_ch

pop_name = ['T', 'E', 'P']

favr = open('tc.avr.' + suffix, 'r')

for ipop in range(1, 3):
    
    fsfr = open('avfcc.sfr.' + pop_name[ipop] + '.' + suffix, 'w')
    fscv = open('avfcc.scv.' + pop_name[ipop] + '.' + suffix, 'w')
    fsch = open('avfcc.sch.' + pop_name[ipop] + '.' + suffix, 'w')
    fsnf = open('avfcc.snf.' + pop_name[ipop] + '.' + suffix, 'w')
    fsfs = open('avfcc.sfs.' + pop_name[ipop] + '.' + suffix, 'w')

    Nval_before = 8 + (ipop-1) * 8 + 0
    one_column_calculate_statistics(favr, fsfr, Nval_before, xval_min_fr)

    Nval_before = 8 + (ipop-1) * 8 + 1
    one_column_calculate_statistics(favr, fscv, Nval_before, xval_min_cv)

    Nval_before = 8 + (ipop-1) * 8 + 5
    one_column_calculate_statistics(favr, fsch, Nval_before, xval_min_ch)

    Nval_before = 8 + (ipop-1) * 8 + 6
    one_column_calculate_statistics(favr, fsnf, Nval_before, xval_min_ch)

    Nval_before = 8 + (ipop-1) * 8 + 7
    one_column_calculate_statistics(favr, fsfs, Nval_before, xval_min_ch)
